...
local min_steps_before_turn = 4
local max_steps_before_turn = 10
...
local function get_neighbors(grid, current)
  ...
  if x > 0 then
    ...
    if current.dir == dir.left and current.steps_after_turn < max_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.left, steps_after_turn = current.steps_after_turn + 1 })
    elseif (current.dir == dir.up or current.dir == dir.down) and current.steps_after_turn >= min_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.left, steps_after_turn = 1 })
    end
  end
  if x < grid.columns - 1 then
    local next_pos = grid.columns * y + (x + 1)
    if current.dir == dir.right and current.steps_after_turn < max_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.right, steps_after_turn = current.steps_after_turn + 1 })
    elseif (current.dir == dir.up or current.dir == dir.down) and current.steps_after_turn >= min_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.right, steps_after_turn = 1 })
    end
  end
  if y > 0 then
    local next_pos = grid.columns * (y - 1) + x
    if current.dir == dir.up and current.steps_after_turn < max_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.up, steps_after_turn = current.steps_after_turn + 1 })
    elseif (current.dir == dir.left or current.dir == dir.right) and current.steps_after_turn >= min_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.up, steps_after_turn = 1 })
    end
  end
  if y < grid.rows - 1 then
    local next_pos = grid.columns * (y + 1) + x
    if current.dir == dir.down and current.steps_after_turn < max_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.down, steps_after_turn = current.steps_after_turn + 1})
    elseif (current.dir == dir.left or current.dir == dir.right) and current.steps_after_turn >= min_steps_before_turn then
      table.insert(neighbors, { pos = next_pos, dir = dir.down, steps_after_turn = 1 })
    end
  end
  return neighbors
end
...
local function encode_node(node)
 return node.pos .. "-" .. node.dir .. "-" .. node.steps_after_turn
end

local function decode_node(str)
  local pos, d, steps_after_turn = string.match(str, "(%d+)-(%d)-(%d+)")
  return { pos = tonumber(pos), dir = tonumber(d), steps_after_turn = tonumber(steps_after_turn) }
end
...
local function a_star(grid, start, goal, h)
  ...
  local start_node = { pos = start, dir = dir.right, steps_after_turn = 0 }
